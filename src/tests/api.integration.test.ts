...
describe('Admin route authentication', () => {
  ...
  it('DELETE /admin/users/:adminId → 401 with anon token only', async () => {
  ...
  it('GET /admin/observability/security-alert-history/stats → 401 with anon token only', async () => {
    const { status } = await request('/admin/observability/security-alert-history/stats');
    expect(status).toBe(401);
  });
  ...
  it('GET /admin/observability/security-alert-history/quality → 401 with anon token only', async () => {
    const { status } = await request('/admin/observability/security-alert-history/quality');
    expect(status).toBe(401);
  });
...
describe('Tier 2 admin authorization hardening', () => {
  ...
  it('DELETE /admin/users/:adminId requires super-admin', async () => {
    ...
    const result = await request('/admin/users/admin_fake', {
      method: 'DELETE',
      headers: adminHeaders(),
    });
...
describe('Admin route success path', () => {
  ...
  it('GET /admin/observability/security-alert-history/stats is role-gated and returns status aggregates', async () => {
    if (!ADMIN_TEST_JWT) {
      if (REQUIRE_ADMIN_SUCCESS) {
        throw new Error('REQUIRE_ADMIN_SUCCESS=true but SUPABASE_ADMIN_TEST_JWT is missing');
      }
    ...
    const result = await request('/admin/observability/security-alert-history/stats?sinceMinutes=60', {
      headers: adminHeaders(),
    });
    ...
    expect(result.status).toBe(200);
    expect(typeof result.body.generatedAt).toBe('string');
    expect(result.body.sinceMinutes).toBe(60);
    expect(typeof result.body.totals?.total).toBe('number');
    expect(typeof result.body.totals?.byStatus?.ok).toBe('number');
    expect(typeof result.body.totals?.byStatus?.warning).toBe('number');
    expect(typeof result.body.totals?.byStatus?.critical).toBe('number');
    expect(typeof result.body.rates?.okPct).toBe('number');
  ...
  it('GET /admin/observability/security-alert-history/quality is role-gated and returns quality metrics', async () => {
    if (!ADMIN_TEST_JWT) {
      if (REQUIRE_ADMIN_SUCCESS) {
        throw new Error('REQUIRE_ADMIN_SUCCESS=true but SUPABASE_ADMIN_TEST_JWT is missing');
      }
    ...
    const result = await request('/admin/observability/security-alert-history/quality?sinceMinutes=120', {
      headers: adminHeaders(),
    });
    ...
    expect(result.body.sinceMinutes).toBe(120);
    expect(typeof result.body.totals?.total).toBe('number');
    expect(typeof result.body.totals?.ok).toBe('number');
    expect(typeof result.body.totals?.warning).toBe('number');
    expect(typeof result.body.totals?.critical).toBe('number');
    expect(typeof result.body.quality?.healthyRatioPct).toBe('number');
    expect(typeof result.body.quality?.noisyRatioPct).toBe('number');
