...
function Transactions({
  ...
  const handleFilterType = (val: string) => { setFilterType(val); };
    ...
    <div className="space-y-6">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-700">
              ...
              {virtualItems.map((virtualRow) => {
                const tx = sorted[virtualRow.index];
                return (
      ...
      <div className="bg-[#252b3d] px-6 py-4 rounded-lg">
        <p className="text-sm text-gray-400">
          {sorted.length} transaction{sorted.length !== 1 ? 's' : ''}{sorted.length !== transactions.length ? ` (filtered from ${transactions.length})` : ''}
        </p>
      </div>
    </div>
  );
