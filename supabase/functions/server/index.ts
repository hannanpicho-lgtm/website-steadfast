...
async function handleStartingSnapshot(c: any) {
  ...
  try {
    ...
    // First, detect and pause old auto-generated products with mismatched images (one-time cleanup, reversible)
    ...
    if (pausedMismatched > 0) {
      console.log(`[auto-top-up] Paused ${pausedMismatched} products with mismatched images (reversible)`);
    ...
    if (tierTaskCandidates.length < minRequired) {
      try {
        ...
        while (generated < deficit) {
          ...
          categoryIdx++;
          const result = generateProductForVipTier(userVipLevel, vipCommission, category, usedNames, usedTypesThisLevel, seed++);
          if (!result) {
            seed += 100;
...
function generateProductForVipTier(
  ...
  for (const typeIdx of unusedFirst) {
    ...
    for (let adjAttempt = 0; adjAttempt < templates.adjectives.length; adjAttempt++) {
      ...
      // Image: ALWAYS use the type-specific image so the photo matches the product name.
      // "Premium Foam Roller" and "Max Foam Roller" both get the foam roller photo — this is correct.
      // We intentionally do NOT use usedImages dedup here because a mismatched image is worse than a shared one.
      const image = templates.imagesByProductType[productType]
        ?? templates.fallbackImagesByCategory[category]
        ?? templates.fallbackImagesByCategory['Electronics'];
